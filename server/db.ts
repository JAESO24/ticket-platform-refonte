...
export async function createOrderRecord(input: typeof orders.$inferInsert) { const db = await getDb(); if (!db) throw new Error("Database unavailable"); return db.insert(orders).values(input); }
export async function listTicketsForUser(userId: number) { const db = await getDb(); if (!db) return []; return db.select().from(tickets).where(eq(tickets.ownerId, userId)).orderBy(desc(tickets.createdAt)); }
export async function findTicketByCode(code: string) { const db = await getDb(); if (!db) return undefined; const result = await db.select().from(tickets).where(eq(tickets.code, code)).limit(1); return result[0]; }
export async function listClaimsForUser(userId: number) { const db = await getDb(); if (!db) return []; return db.select().from(claims).where(eq(claims.userId, userId)).orderBy(desc(claims.createdAt)); }
export async function createClaimRecord(input: typeof claims.$inferInsert) { const db = await getDb(); if (!db) throw new Error("Database unavailable"); return db.insert(claims).values(input); }
