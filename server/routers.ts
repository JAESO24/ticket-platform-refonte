...
export const appRouter = router({
  ...
  promoter: router({
    submitRequest: protectedProcedure.input(z.object({ businessName: z.string().min(2), description: z.string().min(20) })).mutation(async ({ ctx, input }) => { const db = await getDb(); if (!db) throw new Error("Database unavailable"); const result = await db.insert(promoterRequests).values({ userId: ctx.user.id, businessName: input.businessName, description: input.description, status: "pending", eligibilityScore: input.description.length >= 80 ? 85 : 65 }); return { id: result[0].insertId, status: "pending" as const, eligibilityScore: input.description.length >= 80 ? 85 : 65 }; }),
